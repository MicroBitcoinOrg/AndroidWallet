...
import com.microbitcoin.core.coins.FiatType;
import com.microbitcoin.core.coins.Value;
import com.microbitcoin.core.uri.CoinURI;
...
import com.microbitcoin.wallet.util.ThrottlingWalletChangeListener;
...
public class AddressRequestFragment extends Fragment {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.action_new_address:
                createNewAddressDialog.setTargetFragment(this, 0);
                createNewAddressDialog.show(getFragmentManager(), "TAG");
                return true;
    ...
    public static class CreateNewAddressDialog extends DialogFragment {
        ...
        public Dialog onCreateDialog(Bundle savedInstanceState) {
            ...
            final AddressRequestFragment f = (AddressRequestFragment) getTargetFragment();
            if (f.pocket.canCreateFreshReceiveAddress()) {
                ...
                builder.setPositiveButton(R.string.button_ok, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        try {
                            Address newAddress = f.pocket.getFreshReceiveAddress(
                                    f.config.isManualAddressManagement());
                            final String newLabel = viewLabel.getText().toString().trim();

                            if (!newLabel.isEmpty()) {
                                final Uri uri =
                                        AddressBookProvider.contentUri(getActivity().getPackageName(), f.type)
                                                .buildUpon().appendPath(newAddress.toString()).build();
                                final ContentValues values = new ContentValues();
                                values.put(AddressBookProvider.KEY_LABEL, newLabel);
                                f.resolver.insert(uri, values);
                            }
                            f.updateView();
    ...
    DialogFragment createNewAddressDialog = new CreateNewAddressDialog();
//            = new DialogFragment() {
//        @Override
//        public Dialog onCreateDialog(Bundle savedInstanceState) {
//            Dialog dialog;
