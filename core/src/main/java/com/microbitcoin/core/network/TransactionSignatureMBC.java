...
import com.microbitcoin.core.Preconditions;
...
public class TransactionSignatureMBC extends ECKey.ECDSASignature {
     ...
     */
    public TransactionSignatureMBC(BigInteger r, BigInteger s) {
        this(r, s, TransactionMBC.SigHash.ALL.value);
    }

    /**
     * Constructs a signature with the given components and raw sighash flag bytes (needed for rule compatibility).
     */
    public TransactionSignatureMBC(BigInteger r, BigInteger s, int sighashFlags) {
        super(r, s);
        this.sighashFlags = sighashFlags | TransactionMBC.SigHash.FORKID.value;
    }

    /**
     * Constructs a transaction signature based on the ECDSA signature.
     */
    public TransactionSignatureMBC(ECKey.ECDSASignature signature, TransactionMBC.SigHash mode, boolean anyoneCanPay) {
        super(signature.r, signature.s);
        sighashFlags = calcSigHashValue(mode, anyoneCanPay, true);
    }

    /**
     * Constructs a transaction signature based on the ECDSA signature for Bitcoin Cash (BCC).
     */
    public TransactionSignatureMBC(ECKey.ECDSASignature signature, TransactionMBC.SigHash mode, boolean anyoneCanPay, boolean forkIdEnabled) {
        super(signature.r, signature.s);
        sighashFlags = calcSigHashValue(mode, anyoneCanPay, true);
    }
     ...
     */
    public static TransactionSignatureMBC dummy() {
        BigInteger val = ECKey.HALF_CURVE_ORDER;
        return new TransactionSignatureMBC(val, val);
    }

    /**
     * Calculates the byte used in the protocol to represent the combination of mode and anyoneCanPay.
     */
    public static int calcSigHashValue(TransactionMBC.SigHash mode, boolean anyoneCanPay) {
        Preconditions.checkArgument(TransactionMBC.SigHash.ALL == mode || TransactionMBC.SigHash.NONE == mode || TransactionMBC.SigHash.SINGLE == mode); // enforce compatibility since this code was made before the SigHash enum was updated
        int sighashFlags = mode.value;
        if (anyoneCanPay)
            sighashFlags |= TransactionMBC.SigHash.ANYONECANPAY.value;
        return sighashFlags;
    }

    public static int calcSigHashValue(TransactionMBC.SigHash mode, boolean anyoneCanPay, boolean forkIdEnabled) {
        int sighashFlags = calcSigHashValue(mode, anyoneCanPay);
        if (forkIdEnabled) {
            sighashFlags |= TransactionMBC.SIGHASH_FORKID;
        }
        return sighashFlags;
    }
    ...
    public static boolean isEncodingCanonical(byte[] signature) {
        ...
        int hashType = (signature[signature.length - 1] & 0xff) & ~TransactionMBC.SigHash.ANYONECANPAY.value; // mask the byte to prevent sign-extension hurting us

        // ignore FORKID (0x40) for Bitcoin Cash
        hashType = hashType & 0x3f;
        if (hashType < TransactionMBC.SigHash.ALL.value || hashType > TransactionMBC.SigHash.SINGLE.value)
            return false;
    ...
    public boolean anyoneCanPay() {
        return (sighashFlags & TransactionMBC.SigHash.ANYONECANPAY.value) != 0;
    }

    public TransactionMBC.SigHash sigHashMode() {
        final int mode = sighashFlags & 0x1f;
        if (mode == TransactionMBC.SigHash.NONE.value)
            return TransactionMBC.SigHash.NONE;
        else if (mode == TransactionMBC.SigHash.SINGLE.value)
            return TransactionMBC.SigHash.SINGLE;
        else
            return TransactionMBC.SigHash.ALL;
    }
    ...
    public ECKey.ECDSASignature toCanonicalised() {
        return new TransactionSignatureMBC(super.toCanonicalised(), sigHashMode(), anyoneCanPay());
    }
     ...
     */
    @Deprecated
    public static TransactionSignatureMBC decodeFromBitcoin(byte[] bytes,
    ...
    public static TransactionSignatureMBC decodeFromBitcoin(byte[] bytes,
        ...
        if (requireCanonicalSValue && !(sig.s.compareTo(ECKey.HALF_CURVE_ORDER) <= 0))
        ...
        return new TransactionSignatureMBC(sig.r, sig.s, bytes[bytes.length - 1]);
    }
