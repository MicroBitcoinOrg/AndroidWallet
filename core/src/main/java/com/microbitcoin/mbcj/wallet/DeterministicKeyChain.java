 ...
 * {@link com.microbitcoin.mbcj.crypto.DeterministicHierarchy}, to derive all the keys in the keychain from a master seed.
 ...
 * written down on paper or, when represented using a BIP 39 {@link com.microbitcoin.mbcj.crypto.MnemonicCode},
 ...
 * {@link com.microbitcoin.mbcj.crypto.DeterministicKey#serializePubB58(com.microbitcoin.mbcj.core.NetworkParameters)}. The resulting "xpub..." string encodes
 * sufficient information about the account key to create a watching chain via
 * {@link com.microbitcoin.mbcj.crypto.DeterministicKey#deserializeB58(com.microbitcoin.mbcj.crypto.DeterministicKey, String, com.microbitcoin.mbcj.core.NetworkParameters)}
 * (with null as the first parameter) and then
 * {@link DeterministicKeyChain#DeterministicKeyChain(com.microbitcoin.mbcj.crypto.DeterministicKey)}.</p>
 * <p>
 * <p>This class builds on {@link com.microbitcoin.mbcj.crypto.DeterministicHierarchy} and
 * {@link com.microbitcoin.mbcj.crypto.DeterministicKey} by adding support for serialization to and from protobufs,
...
public class DeterministicKeyChain implements EncryptableKeyChain {
     ...
     * The returned key can be serialized and then passed into {@link #watch(com.microbitcoin.mbcj.crypto.DeterministicKey)}
    ...
    /* package */ List<ECKey> getKeys(boolean includeLookahead) {
        ...
        if (!includeLookahead) {
            ...
            for (ECKey key : keys) {
                ...
                if (parent.equals(internalParentKey) && detkey.getChildNumber().i() >= issuedInternalKeys)
                    continue;
                if (parent.equals(externalParentKey) && detkey.getChildNumber().i() >= issuedExternalKeys)
                    continue;
     ...
     *
     * @see com.microbitcoin.mbcj.wallet.MarriedKeyChain
     */
